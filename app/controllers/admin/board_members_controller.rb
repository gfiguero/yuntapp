module Admin
  class BoardMembersController < ApplicationController
    ...
    def search
      scope = current_neighborhood_association.board_members
      @board_members = params[:items].present? ? scope.filter_by_id(params[:items]) : scope
    ...
    def filter_params
      params.permit(:id, :position, :active).compact_blank
    end
