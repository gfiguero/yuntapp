...
| BR-013 | Normalización | El teléfono se normaliza a formato `+569XXXXXXXX`. Si ingresa `9XXXXXXXX` (9 dígitos), se agrega `+56` automáticamente. **Una entrada que no calza con ninguna forma reconocible se conserva tal como la escribió el usuario, para que la validación pueda rechazarla.** Enforcement corregido 2026-08-20: la normalización guardaba el resultado de `gsub(/[^0-9+]/, "")`, que ante un valor sin dígitos (`"no-es-un-telefono"`) era la cadena vacía; como las validaciones de teléfono corren con `allow_blank`/`if: phone.present?`, no llegaban a ejecutarse y el registro se guardaba **sin teléfono, en silencio**, con el usuario creyendo que lo había registrado. La lógica, que estaba triplicada en `VerifiedIdentity`, `IdentityVerificationRequest` y `AdministrationRequest`, vive ahora en el concern `PhoneNormalization` |
...
| BR-021 | Residencia | **[RETIRADA 2026-08-19 — reemplazada por BR-150]** Decía: "El primer residente aprobado de un domicilio recibe `household_admin: true` en su `Residency`. Los siguientes no". Se escribió cuando un domicilio equivalía a un núcleo familiar. Desde BR-040/BR-041 un `HouseholdUnit` aloja varios `FamilyGroup`, cada uno con su propio `household_admin`, y el criterio dejó de ser el orden de llegada |
| BR-022 | Acceso | Solo el `household_admin` puede solicitar certificados y agregar nuevos miembros al hogar. **Ámbito: su propio `FamilyGroup`, no el `HouseholdUnit` completo** (precisado 2026-08-19; ver BR-041 y BR-098) |
...
| BR-026 | Acceso | **[CORREGIDA 2026-08-20]** Decía: "Un `Member` rechazado puede re-enviar su solicitud cambiando el estado de vuelta a `pending`". Ese re-envío nunca existió y contradecía BR-047 (el rechazo permanece intacto en el historial). El re-envío real es por **duplicación**: se crea una `OnboardingRequest` nueva en `draft` con los datos copiados y la rechazada no se toca (BR-048/BR-049) |
...
| BR-032 | Acceso | **[RETIRADA 2026-08-19 — reemplazada por BR-151]** Decía: "El `household_admin` es el único residente del domicilio que tiene cuenta de usuario". Falso desde BR-040: un domicilio con dos `FamilyGroup` tiene dos usuarios con login, uno por núcleo |
...
| BR-034 | Residencia | Si el `household_admin` abandona el domicilio (reinicia onboarding o se va a otra junta), los residentes dependientes **de su `FamilyGroup`** quedan desvinculados. No se migran automáticamente a otro `household_admin`, ni siquiera a uno de otro núcleo del mismo domicilio (ámbito precisado 2026-08-19) |
| BR-035 | Onboarding | Para que exista un nuevo `household_admin`, ese residente debe hacer su propio onboarding completo. La aprobación le crea **su propio `FamilyGroup`** dentro del domicilio (BR-043), no lo coloca al mando del núcleo anterior. Una vez aprobado, vuelve a registrar a sus residentes dependientes (ámbito precisado 2026-08-19) |
...
| BR-041 | Residencia | Cada `FamilyGroup` tiene su propio `household_admin` que gestiona únicamente sus residentes dependientes. No puede editar ni ver los residentes de otro `FamilyGroup` dentro del mismo `HouseholdUnit`. **Enforcement**: `Panel::DependentsController` filtra por `family_group` en todas sus consultas; `Panel::ResidenceCertificatesController#selectable_residencies` hace lo propio y es la única fuente de titulares, también para `create` (agregado 2026-08-19 — antes ofrecía todo el `HouseholdUnit`, permitiendo ver y emitir a nombre de otra familia conviviente). **Enforcement completado 2026-08-21**: aquel fix cerró la emisión pero dejó `index`, `show` y `download` scopeados por `household_unit`, así que la fuga siguió abierta en **lectura** —el lado más grave, porque el PDF que sirve `download` lleva el RUN sin enmascarar y el domicilio completo—. Las tres acciones pasan por `own_family_group_certificates`, que filtra por el `family_group` del solicitante. Lo detectaron tres auditores independientes en la auditoría del 2026-08-21 (`docs/2026-08-21-auditoria-go-live.md`); ningún test cubría esas acciones, que es por qué el fix anterior pareció completo |
| BR-042 | Residencia | Un `household_admin` puede visualizar qué otros `FamilyGroup` existen en su mismo `HouseholdUnit`, pero solo en modo lectura, para tener contexto de quiénes conviven en el domicilio. Implementado 2026-08-20 en `Panel::HouseholdNeighboursController#index` (`/panel/household_neighbours`). **Alcance precisado**: la vista muestra **únicamente recuentos** —cuántos núcleos hay en la dirección y cuántas personas integra cada uno—, nunca nombres ni RUN. La redacción original ("quiénes conviven") chocaba con BR-041, que prohíbe ver los residentes de otro núcleo; se resolvió con el mismo criterio del PR #159: entre contexto y aislamiento, gana el aislamiento. Decisión de producto del owner |
...
| BR-047 | Onboarding | Una solicitud rechazada permanece en el historial del usuario con el motivo de rechazo visible. No se destruye ni archiva. Implementado 2026-08-20: `Panel::OnboardingRequestsController#index` (`/panel/onboarding/history`), enlazado en el menú del panel cuando hay solicitudes resueltas. Antes la regla no se cumplía: el panel solo consultaba `current_onboarding_request`, que abarca `draft` y `pending`, así que una solicitud rechazada **desaparecía de la vista del usuario junto con su motivo** |
...
| BR-049 | Onboarding | Al duplicar una solicitud rechazada, se crea una nueva `OnboardingRequest` en estado `draft` con los datos copiados. La solicitud original rechazada permanece intacta en el historial. Implementado 2026-08-20 vía `OnboardingRequest#duplicate!`, transaccional (BR-017): copia OR + IVR + RVR o no copia ninguna. **Los documentos adjuntos NO se copian** — decisión de producto: el rechazo suele deberse justamente a un documento ilegible o incorrecto, y recopiarlo invitaría a reenviar el mismo problema. Tampoco se hereda `terms_accepted_at` (BR-015: los términos se aceptan al enviar). Duplicable solo desde `rejected` o `cancelled`, y solo si el usuario no tiene ya una solicitud activa — el wizard asume una sola `current_onboarding_request` |
...
| BR-078 | Validación | El RUN del titular se muestra parcialmente oculto en la verificación pública (formato `1.XXX.XXX-K`) para proteger privacidad. La verificación pública no expone datos completos del titular. Enforcement corregido 2026-08-20: `masked_run` hacía `return raw` cuando el RUN no se podía separar en cuerpo y dígito verificador, así que un valor sin guión —dato legacy o mal migrado— se publicaba **entero** en `/verify/:identifier`, que es pública y sin login. Ahora **falla cerrado**: todo RUN que no venga en el formato normalizado de BR-010 se enmascara por completo (`X.XXX.XXX-X`) |
...
| BR-093 | Acceso | El email de la cuenta de usuario es inmutable después del registro. La vista "Mi Cuenta" (`users/registrations/edit`) lo muestra deshabilitado y `Users::RegistrationsController` descarta el parámetro `email` en el update (defensa server-side ante formularios manipulados). Quien necesite otro correo debe registrar una cuenta nueva; la identidad verificada se traslada mediante el mecanismo de RUN duplicado (BR-057–BR-059). Enforcement reforzado 2026-08-20: la regla vivía **solo** en ese controller, o sea que dependía de que cada nuevo controller recordara descartar el parámetro. Ahora `User` valida `email_is_immutable` `on: :update`, cerrando la ruta por consola y cualquier controller futuro. En la misma pasada se eliminó `Admin::UsersController` y sus vistas: era código muerto (sin `resources :users` en el namespace `admin`) que permitía `:email` y el toggle `:admin` en sus params — habría violado esta regla y la gobernanza de admins (BR-122) apenas alguien le agregara una ruta |
...
| BR-098 | Certificados | El `household_admin` puede solicitar certificados **a nombre de** cualquier residente dependiente **de su `FamilyGroup`** (corregido 2026-08-19: decía "de su domicilio", lo que contradecía BR-041 y habilitaba emitir a nombre de otra familia que conviviera en la misma dirección); el titular del certificado es el `Member(dependent: true)` del residente, no el admin. Esto matiza BR-022/BR-033: el dependiente nunca opera ni inicia sesión, pero sí puede ser titular de un certificado gestionado por el admin (crítico para dependientes adultos —mayores, personas con discapacidad— que requieren su propio certificado de residencia). Implementado en `Panel::ResidenceCertificatesController`: `selectable_residencies` ofrece todo residente **del `family_group` del solicitante** con `Member` aprobado en la junta, y `create` **reusa esa misma lista** para resolver el titular, de modo que un `member_id` manipulado no eluda el filtro. El dependiente debe tener `Member` aprobado y activo — dependientes desactivados (BR-037) quedan excluidos del selector |
...
| BR-100 | Integridad | **Ningún dato consolidado se destruye.** La información generada en la plataforma (identidades verificadas, socios, residencias, certificados emitidos, directiva, publicaciones) debe sobrevivir a errores, cambios y "borrados". Un `Member`, una vez creado, **nunca** se destruye: solo se desactiva (`deactivate!` → `inactive`), conservando su historial. Enforcement: guard `before_destroy` en `Member` que aborta la destrucción por cualquier ruta; `residence_certificates`/`board_members` con `dependent: :restrict_with_error` (no `:destroy`); sin acción `destroy` en `admin/members`; y el restablecimiento de cuenta (`panel/account_resets`) desactiva y desvincula en vez de destruir. Los borrados físicos que aún existan (p. ej. cascadas `dependent: :destroy` en `NeighborhoodAssociation` — issue #90) deben migrarse a este modelo. **Excepción única y cerrada — reseteo del entorno de pruebas, 2026-08-21**: se borraron los 7 certificados emitidos, sus PDFs y el único `PaymentEvent` de producción, junto con el formato de folio anterior que arrastraban (BR-006). Era data de prueba sin ningún cliente real: el owner era el único usuario y las 26 cuentas eran alias suyos. Se hizo con respaldo previo (`production.sqlite3.bak-prereset-certs`) y sin tocar socios, identidades, onboarding, residencias ni domicilios, verificado con conteo antes y después. **La excepción se cierra con el primer certificado emitido a un vecino real**; desde entonces esta regla aplica sin excepciones y un reseteo equivalente deja de ser posible. Se documenta porque la regla no distingue entre producción real y entorno de pruebas, y un borrado sin constancia haría parecer que la regla es decorativa |
...
| BR-127 | Identidad | Datos del dirigente con las mismas normalizaciones/validaciones del residente: RUN+DV (BR-010/011), teléfono +569 (BR-013), nombres capitalizados (BR-014). Enforcement completado 2026-08-20: `AdministrationRequest` normalizaba el teléfono pero **nunca validaba su formato** (solo presencia al pasar a `pending`), a diferencia de `VerifiedIdentity` e `IdentityVerificationRequest`. El defecto de BR-013 lo mantenía oculto: la basura quedaba vacía y fallaba por presencia, así que el hueco solo se hacía visible al corregir la normalización |
...
| BR-150 | Residencia | **El `household_admin` se determina por la vía de entrada, no por el orden de llegada al domicilio** (reemplaza BR-021). Quien completa un onboarding propio y es aprobado recibe `Residency(household_admin: true)` en un `FamilyGroup` nuevo dentro del `HouseholdUnit` (BR-043), sea el primero o el quinto en esa dirección. Quien es registrado como residente dependiente recibe `household_admin: false` (BR-067). Un domicilio con tres familias tiene tres `household_admin`, uno por núcleo |
| BR-151 | Acceso | **Cada `FamilyGroup` tiene exactamente un residente con cuenta de usuario: su `household_admin`** (reemplaza BR-032). Los demás miembros del núcleo son residentes dependientes, registrados por RUN y sin login (BR-065). Un `HouseholdUnit` con varios núcleos tiene por tanto varios usuarios con cuenta —uno por núcleo—, cada uno sin visibilidad sobre los residentes de los otros (BR-041) |
| BR-152 | Certificados | **Cada certificado registra quién lo solicitó** (`residence_certificates.requested_by_id`), que no siempre es su titular: por BR-098 el `household_admin` puede pedirlo a nombre de un residente dependiente de su núcleo. El solicitante se toma **siempre de la sesión**, nunca del formulario. Antes el certificado solo guardaba al titular (`member`), así que no había forma de auditar quién lo pidió — fue justamente lo que impidió responder de forma concluyente si alguna emisión había cruzado núcleos familiares antes del fix de BR-041 (PR #159). La columna es **nullable a propósito**: los certificados emitidos antes de esta regla no tienen solicitante registrable y no se les inventa uno; un backfill con el titular sería una afirmación falsa justo en los casos que interesa distinguir. La vista del admin muestra el solicitante solo cuando difiere del titular (`requested_by_other?`) |
| BR-153 | Integridad | **Los documentos que sube el vecino se validan en tipo y tamaño.** Máximo **10 MB** por archivo y solo formatos de foto o PDF (`image/jpeg`, `image/png`, `image/webp`, `image/heic`, `image/heif`, `application/pdf`). HEIC se acepta porque es lo que sube un iPhone por defecto y rechazarlo dejaría fuera a buena parte de los vecinos. Aplica a todo adjunto de origen externo —documentos de identidad, comprobantes de domicilio, certificado de vigencia de la directiva— vía el concern `AttachmentLimits`; **no** aplica al PDF del certificado, que lo genera la plataforma. Antes no existía ninguna validación: Active Storage escribe en el mismo disco que la base SQLite, así que una subida suficientemente grande llenaba el disco y dejaba a la aplicación sin poder escribir, y nada impedía adjuntar un ejecutable. Detectado en la auditoría del 2026-08-21 |
